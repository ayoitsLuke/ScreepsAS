...
exports.calcCreepCost = function (body) {
    ...
    body.forEach((i) => {
        if (_.isObject(i)) {
            result += BODYPART_COST[i.type];
        }
        else {
            result += BODYPART_COST[i];
        }
    });
    return result;
};
exports.checkConstructionSite = function (objects, structureType, x, y) {
    ...
    if (structureType != 'road' && structureType != 'container' && (x == 1 || x == 48 || y == 1 || y == 48)) {
        if (x == 1)
            borderTiles = [[0, y - 1], [0, y], [0, y + 1]];
        if (x == 48)
            borderTiles = [[49, y - 1], [49, y], [49, y + 1]];
        if (y == 1)
            borderTiles = [[x - 1, 0], [x, 0], [x + 1, 0]];
        if (y == 48)
            borderTiles = [[x - 1, 49], [x, 49], [x + 1, 49]];
    }
...
exports.getDiff = function (oldData, newData) {
    ...
    var result = {}, oldIndex = getIndex(oldData), newIndex = getIndex(newData);
...
exports.encodeTerrain = function (terrain) {
    ...
    for (var y = 0; y < 50; y++) {
        for (var x = 0; x < 50; x++) {
            var objects = _.filter(terrain, { x, y }), code = 0;
...
exports.calcTotalReactionsTime = function (mineral) {
    const reagents = _.reduce(REACTIONS, (a, n, j) => { _.forEach(n, (k, v) => a[k] = [v, j]); return a; }, {});
