...
class Traveler {
  ...
  static travelTo(creep, destination, options = {}) {
    ...
    let rangeToDestination = creep.pos.roomName === destination.roomName ? creep.pos.getRangeTo(destination) : Infinity; // Monkey patch, due to overwriting RoomPosition.getRangeTo()
    ...
    if (!creep.memory._trav) {
      creep.memory._travel = undefined;
      creep.memory._trav = {};
    }
  ...
  static getCreepMatrix(room) {
    if (!this.creepMatrixCache[room.name] || Game.time !== this.creepMatrixTick) {
      ...
      this.creepMatrixCache[room.name] = Traveler.addCreepsToMatrix(room, this.getStructureMatrix(room, true)
        .clone());
    }
  ...
  static patchMemory(cleanup = false) {
    ...
    for (let roomName in Memory.empire.hostileRooms) {
      ...
      if (cleanup) {
        Memory.empire.hostileRooms[roomName] = undefined;
      }
    }
    if (cleanup) {
      Memory.empire.hostileRooms = undefined;
    }
