...
exports.roomNameToXY = function(name) {
  const chars = name.toUpperCase().split(/(\d+)/); // "W99N99" -> ["W", "99", "N", "99", ""]
...
exports.calcTotalReactionsTime = function(mineral) {
  ...
  const calcStep = m =>
    !!REACTION_TIME[m] ? REACTION_TIME[m] + calcStep(reagents[m][0]) + calcStep(reagents[m][1]) : 0;
...
makeMap = function(runtimeData, register, globals) {
  ...
  return {
    ...
    isRoomProtected(roomName) {
      register.deprecated(
        "Method `Game.map.isRoomProtected` is deprecated and will be removed. Please use `Game.map.isRoomAvailable` instead.",
      );
      if (!/^(W|E)\d+(N|S)\d+$/.test(roomName)) {
        return null;
      }
