Object.defineProperties(Structure.prototype, {
    area: {
        ...
        get: function () {
            //stub
            if (this.pos.inRangeToRoomObject(this.room.controller, 4))
                return "controller";
            if (this.pos.findInRange(FIND_SOURCES, 2)[0])
                return "source";
            if (this.pos.findInRange(FIND_MY_STRUCTURES, 1, {
                filter: s => s.structureType === STRUCTURE_EXTENSION
            })[0])
                return "outpost";
            if (this.pos.findInRange(FIND_MINERALS, 1)[0])
                return "mineral";
        }
    }
});
...
Structure.prototype.generateRequests = function () {
    ...
    switch (this.structureType) {
        case STRUCTURE_CONTROLLER:
            ...
            if (this.room.type === "my" && (this.level < 8 || this.ticksToDowngrade < CONTROLLER_DOWNGRADE_SAFEMODE_THRESHOLD))
                requests.push({
        ...
        case STRUCTURE_LINK: // No need unless war time
            if (this.area !== "source" && this.area !== "controller" && this.energyCapacity - this.energy)
        ...
        case STRUCTURE_POWER_SPAWN:
            ...
            if (this.powerCapacity - this.power)
                requests.push(newRequest("transfer", {
        ...
        case STRUCTURE_NUKER:
            ...
            if (this.ghodiumCapacity - this.ghodium)
                requests.push(newRequest("transfer", {
